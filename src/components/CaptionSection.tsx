...
export function CaptionSection() {
  ...
  const [withTimestamp, setWithTimestamp] = useState(false); // 時間碼預設關閉
  ...
  useEffect(() => {
    // Reset when video changes and auto-select author uploaded captions
    ...
    if (availableCaptions && availableCaptions.length > 0) {
      // 尋找作者上傳的字幕（非自動生成且非CC）
      const authorCaption = availableCaptions.find(
        track => !track.isAutoGenerated && !track.isCC
      );
      
      if (authorCaption) {
        setSelectedLang(authorCaption.code);
        toast({
          title: "✨ 已自動選擇",
          description: `作者上傳的字幕: ${authorCaption.lang}`,
          duration: 2000,
        });
      } else {
        // 如果沒有作者上傳的，選擇第一個可用的字幕
        const firstCaption = availableCaptions[0];
        if (firstCaption) {
          setSelectedLang(firstCaption.code);
        }
      }
    }
  }, [currentVideo?.id, availableCaptions, toast]);
